...
Solidsearch is a scalable web search engine. Features include:

* Distributed web crawling (Bot)
** Powerful sql based web page and domain analysis
** Analyze and rate quality for each document of a domain
** Also designed to handle big sites  
** Adaptive crawling speed
** Restful API
** Small memory and CPU footprint
* Crawler / index management (Manager)
** Elasticsearch search index management and control
** Control of multiple crawler instances
** Aggregate and import crawling data to Elasticsearch
** Configure crawling and update behaviour
* User search engine representation (Eearchhead)
** Mapping of user requests to search index
** Delivering SERPs based on search query to user
...
Bot uses very safety crawling speed which does not bring any server in trouble. 

Bot also decreases crawling speed automatically if site is getting slower.

Due to the vary safety speed you will need to wait some time until crawling is finished.
